...
import { BRAND_STATUS_FILTER_OPTIONS } from "@/constants/brand";
import { cn } from "@/lib/utils";
import type { BrandListStatusFilter } from "@/types/brand";
...
interface BrandFiltersProps {
  ...
  onStatusChange: (value: BrandListStatusFilter) => void;
  onClear: () => void;
  onRefresh: () => void;
}
...
export function BrandFilters({
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
      ...
      <Select value={status} onValueChange={(value) => onStatusChange(value as BrandListStatusFilter)}>
        <SelectTrigger className="h-9 w-full focus:border-primary focus:ring-0 sm:w-44">
          <SelectValue placeholder="Trạng thái" />
        </SelectTrigger>
        <SelectContent>
          {BRAND_STATUS_FILTER_OPTIONS.map((option) => (
