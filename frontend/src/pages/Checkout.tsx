...
export default function Checkout() {
  ...
  const canPlaceOrder = purchasableLines.length > 0 && !hasCartIssues && !productsQuery.isLoading && !productsQuery.isError;
  ...
  const placeOrder = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!canPlaceOrder) {
      toast.error("Vui lòng kiểm tra lại sản phẩm trong giỏ trước khi đặt hàng");
      return;
    }
    ...
    const orderItems = purchasableLines.map((line) => ({
      ...
      name: line.product?.name ?? `Sản phẩm #${line.productId}`,
      quantity: line.quantity,
      price: line.product ? productPrice(line.product) : 0,
    }));
              ...
              <div className="rounded-xl border border-stone-200 bg-white p-6">
                ...
                <ul className="space-y-3 border-b border-stone-200 pb-4">
                  {lines.map((line) => {
                    ...
                    const lineTotal = product ? productPrice(product) * line.quantity : 0;
